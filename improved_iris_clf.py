...
dt = st.sidebar.file_uploader("Use a custom CSV file")

if dt is not None:
    # st.write("CSV")
    cust_df = pd.read_csv(dt)
    for i in range(cust_df.shape[0]):
        cust_feat = cust_df.iloc[i,:]
        species_type = prediction(svc_model, cust_df[0], cust_df[1], cust_df[2], cust_df[3])
        ...
        if species_type == 'Iris-versicolor':
            ...
            st.success(f"Accuracy score of this model is: {score}")
            st.balloons()
